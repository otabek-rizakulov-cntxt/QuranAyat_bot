...
class InMemoryHifzStore(HifzStore):
  ...
  async def add_interval(self, user_id, surah, start_ayah, end_ayah):
    ...
    if len(overlapping) == 1 and (overlapping[0].start_ayah <= start
                                  and overlapping[0].end_ayah >= end):
  ...
  async def remove_range(self, user_id, surah, start_ayah, end_ayah):
    ...
    affected = [r for r in self._rows(user_id, surah)
                if r.start_ayah <= end and r.end_ayah >= start]
    replacements = []
    for row in affected:
      self._state.hifz_interval.remove(row)
...
class PostgresHifzStore(HifzStore):
  ...
  explicit transaction with `FOR UPDATE` on the affected rows, so two concurrent
  marks on the same surah cannot both read the pre-merge state and leave
  overlapping rows behind.
  """
